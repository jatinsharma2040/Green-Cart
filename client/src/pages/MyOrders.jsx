        ...
        <div key={index} className='border border-gray-300 rounded-lg mb-10 p-4 py-5 max-w-4xl'>
            ...
            {order.items.map((item, index) =>(
                ...
                <div key={index} className={`relative bg-white text-gray-500/70 ${order.items.length !== index + 1
                    && 'border-b'} border-gray-300 flex flex-col md:flex-row md:items-center jusify-between
                    ...
                    <div className='text-gray-800/40 text-lg font-medium ml-25'>
                    <p>Quantity: {item.quantity || '1'}</p>
                    <p>Status: {order.status}</p>
                    <p>Date: {new Date(order.createdAt).toLocaleDateString()}</p>
                    </div>

                    <p className='text-primary text-lg font-medium ml-15'>
